...
const ALL_GRADES = ['Membru PR', 'Adjunct PR', 'Manager PR', 'Supervizor PR', 'Conducere Spital'];
const GRADE_OPTIONS = ALL_GRADES;

// Gradele care au acces la editare
...
const GRADE_SECTION_LABEL = {
  'Membru PR':       'Relații Publice',
  'Adjunct PR':      'Relații Publice',
  'Manager PR':      'Relații Publice',
  'Supervizor PR':   'Supervizor PR',
  'Conducere Spital':'Conducere Spital',
};
...
export default function MembersPage() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (stored) {
          ...
          if (session.discord_id === editTarget.discord_id) {
  ...
  // Group by section
  const prMembers   = filtered.filter(m => ['Membru PR','Adjunct PR','Manager PR'].includes(m.rank));
  const supMembers  = filtered.filter(m => m.rank === 'Supervizor PR');
  const hospMembers = filtered.filter(m => m.rank === 'Conducere Spital');
      ...
      <main className={styles.main}>
        <header className={styles.header}>
          ...
          <div className={styles.headerSearch}>
            ...
            <input
              ...
              placeholder="Caută după nume, grad, callsign, ID..."
              value={search}
        ...
        {loading ? (
          <div className={styles.loadState}>
            ...
            <span>Se încarcă membrii...</span>
          </div>
        ) : (
          <div className={styles.allSections}>
            <MemberSection title="Relații Publice" members={prMembers}   accent="#8b5cf6" canEdit={canEdit} onEdit={openEdit} />
            {supMembers.length > 0  && <MemberSection title="Supervizori PR"   members={supMembers}  accent="#6366f1" canEdit={canEdit} onEdit={openEdit} />}
            {hospMembers.length > 0 && <MemberSection title="Conducere Spital" members={hospMembers} accent="#3b82f6" canEdit={canEdit} onEdit={openEdit} />}
          </div>
        )}
      </main>
          ...
          <div className={styles.modal} onClick={e => e.stopPropagation()}>
              ...
              <div className={styles.modalProfile}>
                ...
                <div>
                  ...
                  <div className={styles.modalSub}>
                    <span className={styles.modalRankBadge} style={{ '--a': GRADE_ACCENT[editTarget.rank] || '#8b5cf6' }}>
                      {editTarget.rank}
                    </span>
                  </div>
                </div>
            ...
            <div className={styles.formGrid}>
              <div className={styles.field}>
                ...
                <input className={styles.input} value={editForm.full_name}
                  onChange={e => setEditForm(f => ({ ...f, full_name: e.target.value }))} />
              </div>

              <div className={styles.field}>
                <label className={styles.label}>Grad</label>
                <select className={styles.select} value={editForm.rank}
              ...
              <div className={styles.field}>
                ...
                <input className={styles.input} placeholder="ex: PR-7"
                  ...
                  onChange={e => setEditForm(f => ({ ...f, callsign: e.target.value }))} />
              </div>
              ...
              <div className={`${styles.field} ${styles.fieldFull}`}>
                ...
                <textarea className={styles.textarea} placeholder="Observații..."
...
function MemberSection({ title, members, accent, canEdit, onEdit }) {
  if (members.length === 0) return null;
  return (
    <div className={styles.section}>
      <div className={styles.sectionHeader} style={{ '--a': accent }}>
      ...
      <div className={styles.tableWrap}>
        <table className={styles.table}>
          ...
          <tbody>
            {members.map((m, i) => {
              const sc = STATUS_COLORS[m.status] || STATUS_COLORS['Inactiv'];
              ...
              return (
                <tr key={m.id} className={styles.row} style={{ animationDelay: `${i * .03}s` }}>
                  ...
                  <td>
                    <span className={styles.gradeBadge} style={{ '--ga': ga }}>
                      {m.rank}
                    </span>
                  </td>
                  <td>
