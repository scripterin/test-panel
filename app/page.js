...
function getRedirectUri() {
  ...
  return window.location.hostname === 'localhost'
    ...
    : `${process.env.NEXT_PUBLIC_SITE_URL || 'https://localhost:3000'}/auth/callback`;
}

export default function LoginPage() {
  const params          = useSearchParams();
  const router          = useRouter();
  const [state, setState] = useState('idle'); // idle | denied | error
    ...
    <main className={styles.root}>
      ...
      <div className={styles.card}>
        ...
        <div className={styles.divider}>
          ...
          <span className={styles.dividerText}>autentificare</span>
          <span className={styles.dividerLine} />
        </div>
      ...
      <div className={styles.footer}>
        Panel PR &copy; {new Date().getFullYear()} · Sistem Intern
      </div>
    </main>
  );
}
